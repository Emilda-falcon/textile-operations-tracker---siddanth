...
document.addEventListener('click',event=>{
  ...
  ['scan-entry-fields','scan-entry-type','partial-row','scan-note'].forEach(id=>{const item=document.getElementById(id);if(item){item.style.opacity='1';item.style.pointerEvents='';}});
  document.getElementById('entry-submit').disabled=false;
  document.querySelector('[data-show-entry-options]')?.removeAttribute('hidden');
