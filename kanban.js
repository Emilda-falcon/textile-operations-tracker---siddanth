...
document.addEventListener('change',event=>{
  if(event.target.id!=='entry-type')return;
  const note=document.getElementById('entry-type-note');
  if(!note)return;
  if(event.target.value==='rework'){
    note.className='callout amber';
    note.style.display='block';
    note.innerHTML='<strong>Rework record:</strong> enter the corrective meters and reason. This is linked to the original stage and is not counted as fresh production output.';
  }else{
    note.className='callout green';
    note.style.display='block';
    note.textContent=event.target.value==='partial'?'Only the verified completed meters will be recorded for this partial entry.':'This entry will be recorded as normal production output.';
  }
},true);
